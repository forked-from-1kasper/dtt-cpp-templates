...
template<template<typename> typename Φ> struct Reassoc {
    template<typename> struct LambdaM;

    template<Literal k, typename T> struct LambdaM<Assoc<k, T>>
    { using value = Assoc<k, Φ<T>>; };

    template<typename T> using Lambda = LambdaM<T>::value;
};